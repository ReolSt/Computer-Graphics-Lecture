...
public:
	GLPhysics()
	{
		...
		this->DebugMeshRenderer->Mesh->SetDrawMode(GLMeshDrawMode::Line);
	...
	void Render(const glm::mat4& cameraMatrix)
	{
		...
		auto mesh = this->DebugMeshRenderer->Mesh;
		mesh->ClearVertices();
		mesh->ClearIndices();
		...
		for (const auto& line : lines)
		{
			...
			GLPoint p1({ line.point1.x, line.point1.y, line.point1.z }, { color1, color1, color1, 1.0f });
			GLPoint p2({ line.point2.x, line.point2.y, line.point2.z }, { color2, color2, color2, 1.0f });
		...
		if (index > 0)
		{
			this->DebugMeshRenderer->Render(glm::mat4(1.0f), cameraMatrix);
		}
	}
