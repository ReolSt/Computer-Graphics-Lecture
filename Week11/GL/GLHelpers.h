...
{
	RandVertexColor3f(gameObject->MeshRenderer->GetMesh());
}

void RandVertexColor4f(const GLSharedPtr<GLGameObject>& gameObject)
{
	RandVertexColor4f(gameObject->MeshRenderer->GetMesh());
}
...
{
	SetVertexColor(gameObject->MeshRenderer->GetMesh(), color);
}

void RandMaterial(const GLSharedPtr<GLGameObject>& gameObject)
{
	auto material = gameObject->MeshRenderer->GetMaterial();
